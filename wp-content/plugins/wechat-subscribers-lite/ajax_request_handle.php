...
function prefix_ajax_add_foobar(){
    ...
    $current = isset($_GET['cur'])?$_GET['cur']:1;
	    ...
	    if(isset($_GET['key'])&&trim($_GET['key'])!=""){
	    	...
	    	$published_posts = $wpdb->get_results("select ID,post_date,post_title,post_type from wp_posts where (post_status = 'publish' or post_status = 'private') and post_title LIKE '%{$str}%' order by post_title asc limit 30 ");
    ...
    //get posts
      ...
      $end   = $offset+$posts_per_page;
		  $posts_array = $wpdb->get_results("select ID,post_date,post_title,post_type from wp_posts where (post_status = 'publish' or post_status = 'private') and post_title LIKE '%{$str}%' order by post_title asc limit $start,$end");
